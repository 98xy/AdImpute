...
parser.add_argument('--lambda_val', type=int, default=1000, help="Regularization coefficient, to control the contribution of regularization term in the cost function.")
...
# Data
parser.add_argument('--data', type = str, default='74672.csv', help = "Dataset to run the script on. In the paper we choose from : ['52529.csv', '75748.csv', '74672.csv', 'Simulate_v9.csv', 'Simulate_v10.csv']")
...
parser.add_argument('--save_model_location', type=str, default='checkpoints/admodel1.ckpt', help="Location to save the learnt model")
parser.add_argument('--load_model_location', type=str, default='checkpoints/admodel0.ckpt', help="Load the saved model from.")
...
parser.add_argument('--imputed_save', type=str, default='74672out', help="save the imputed matrix as")
parser.add_argument('--masked_save', type=str, default='adout', help="save the masked matrix as")

#Set a tag
parser.add_argument('--tag_matrix_data', type=str, default='adtag74672.csv', help="set the tag matrix.In the paper we choose from : ['adtag52529.csv', 'adtag75748.csv', 'adtag74672.csv', 'adtag9.csv', 'adtag10.csv']")
parser.add_argument('--tag_weight', type=float, default=1, help="set the tag weight")
...
if __name__ == '__main__':
	...
	with open("data/" + FLAGS.tag_matrix_data) as f:
		...
		#print(ncols)
		tag_count_matrix = np.loadtxt(open("data/" + FLAGS.tag_matrix_data, "rb"), delimiter=",", skiprows=1, usecols=range(1,ncols+1))
	...
	rmse_loss =tf.add(tf.pow(tf.norm(y_true - y_pred * mask), 2),tf.multiply(tf.constant(y_tag_weight, dtype="float32"),tf.pow(tf.norm(y_pred * mask - y_tag), 2)))
